...
class PurePursuit : public rclcpp::Node {
   ...
   private:
    ...
    double curr_lookahead;
    double base_velocity;
    double accel_weight;
    bool accel_mode;

    rclcpp::Subscription<sensor_msgs::msg::LaserScan>::SharedPtr lidar_sub;
    std::vector<float> lidar_ranges;  // 라이다 거리값 저장
    double angle_min;                 // 라이다 최소 각도
    double angle_increment;           // 라이다 각도 증분
    ...
    rclcpp::Publisher<std_msgs::msg::Float32>::SharedPtr curr_lookahead_pub;
    ...
    bool isPathClear(double x_start, double y_start, double x_end, double y_end);
    void get_waypoint(Eigen::Vector2d &car_heading);
